...
export function ChatFileUpload({
  ...
  return (
    <div className="p-4 pb-[max(1rem,env(safe-area-inset-bottom))] bg-cream border-t border-ink-200 flex-shrink-0 space-y-4">
      {/* Hidden file input */}
      <input
        ref={fileInputRef}
        type="file"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    {uploaded ? (
                      ...
                      <FileText className="h-5 w-5 text-ink-400 flex-shrink-0" />
                    )}
                    <span className={cn(
                      'font-medium text-sm',
                      uploaded ? 'text-forest-800' : 'text-ink-700'
                    )}>
                      {doc.label}
                    </span>
                  </div>
                  {doc.description && !uploaded && (
                    <p className="text-xs text-ink-600 mt-1 ml-7">{doc.description}</p>
                  )}
                  {uploaded && (
                    <p className="text-xs text-forest mt-1 ml-7 truncate">
                      {uploaded.name}
                    </p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {isUploading ? (
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-full hover:bg-sage-200 text-forest transition-colors"
                      disabled={disabled}
                    >
                      <X className="h-4 w-4" />
                    </button>
                    ...
                    <div className="flex gap-1">
                      ...
                      <button
                        ...
                        disabled={disabled}
                        className="p-2.5 rounded-xl bg-sage-200 text-forest hover:bg-sage transition-colors"
                        title="Take photo"
                      >
                      ...
                      <button
                        ...
                        disabled={disabled}
                        className="p-2.5 rounded-xl bg-sand text-ink-700 hover:bg-ink-200 transition-colors"
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          className={cn(
            'flex-1 bg-forest text-cream hover:bg-forest-800',
